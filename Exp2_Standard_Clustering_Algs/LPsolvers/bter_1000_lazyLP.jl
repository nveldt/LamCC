...
open("bter1000bounds_lazyLup.txt","w") do f
...
for i = 1:19
    ...
    matwrite("bterData/D_btr_lazyLup"*"_$lamshort.mat",Dict( "D" => D))
    matwrite("bterData/c_btr_lazyLup"*"_$lamshort.mat",Dict( "c" => c, "lccbound" => lccbound))
    open("bter1000bounds_lazyLup.txt","a") do f
        write(f, "$lccbound ")
        write(f, "$lamshort ")
        write(f,"\n")
    end
