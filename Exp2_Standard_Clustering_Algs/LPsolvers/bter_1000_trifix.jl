...
open("bter_1000_trifix_bounds.txt","w") do f
...
for i = 10:-1:1
    ...
    Gams = [1/lam 2/lam 3/lam 4/lam 5/lam 6/lam 7/lam 8/lam 9/lam 10/lam]
    ...
    P = zeros(n,n)
    Q = zeros(n,n)
    M = TriangleFix(A,Dgraph,E,F,W,P,Q,Float64(gam),Float64(Etol),Float64(TriTol),lam,"TriFix_$lam\_$gam.txt",gam)
    D = M+M'
    lccbound = sum((A[i,j]-lam)*D[i,j] for i=1:n-1 for j = i+1:n) + lam*(n*(n-1)/2 - numedges)

    println("Done with Lambda = ", lam)

    matwrite("Dbter_1000_tri"*"_$lamshort.mat",Dict( "D" => D))
    matwrite("bter_1000_tri"*"_$lamshort.mat",Dict( "lccbound" => lccbound))
    open("bter_1000_trifix_bounds.txt","a") do f
        write(f, "$lccbound ")
        write(f, "$lamshort ")
        write(f,"\n")
    end
